...
export class InventarioService {
  ...
  async processarArquivo(
    ...
    await this.projetosRepository.update(
      { id: projetoId, usuarioId },
      { status: StatusProjeto.PROCESSANDO },
    );
  ...
  async listarArvores(usuarioId: string, projetoId: string) {
  ...
  async buscarEstatisticas(usuarioId: string, projetoId: string) {
