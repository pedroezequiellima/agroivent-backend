...
_COLUNAS_MAP = {
    "nome_comum": ["NOME COMUM", "ESPECIE", "ESPÉCIE", "NOME", "SPP", "ARVORE"],
    "parcela": ["UA", "PARCELA", "P", "UNIDADE AMOSTRAL", "PP", "TALHAO", "TALHÃO"],
    # Medidas a 1,30m (Peito)
    "cap": ["CAP", "C1,3", "CIRCUNFERENCIA", "CIRCUNFERÊNCIA", "CIRCUF. PEITO"],
    "dap": ["DAP", "D1,3", "DIAMETRO", "DIÂMETRO", "DIAM. PEITO"],
    # Medidas a 0,30m (Base)
    "cnb": ["CNB", "CAB", "C0,3", "CIRCUNFERENCIA BASE"],
    "dnb": ["DNB", "D0,3", "DIAMETRO BASE"],
    # Alturas (Diferenciadas)
    "altura_total": ["HT", "HTOTAL", "H"],
    "altura_comercial": ["HC", "HCOM", "ALTURA DO FUSTE", "ALTURA COMERCIAL"],
    # Qualidade e Situação
    "qualidade_fuste": ["QF", "F", "FORMA", "QUALIDADE"],
    "situacao": ["SITUAÇÃO", "SITUACAO", "SIT", "VITALIDADE"],

    # 🔥 produto
    "produto": ["PRODUTO", "DESTINACAO", "DESTINAÇÃO", "USO"],
    "densidade_madeira": ["DENSIDADE", "DENSIDADE_MADEIRA", "DENS"],

    # 🗺️ GEO
    "coordenada_x": ["X", "LONGITUDE", "COORD_X"],
    "coordenada_y": ["Y", "LATITUDE", "COORD_Y"],
}
...
def _ler_planilha(path: str) -> pd.DataFrame:
    ...
    # Lista de termos que indicam que chegamos no cabeçalho real
...
def _carregar_especies(engine) -> tuple[dict[str, dict[str, Any]], list[str]]:
    ...
    avisos: list[str] = []
    # Executar query e processar resultados em uma única transação
    with engine.begin() as conn:
        ...
        # Processar resultados em memória (sem operações de banco dentro do loop)
        for row in rows:
...
def processar_inventario(config: ProcessamentoConfig) -> dict[str, Any]:
    ...
    for idx, row in df.iterrows():
        ...
        # Regra de negócio: DMC pode ser CAP ou DAP
    ...
    # Usar transação única com bulk insert para evitar múltiplas queries concorrentes
    with engine.begin() as conn:
        ...
        if arvores_insert:
            conn.execute(
                ...
                arvores_insert,  # Bulk: array completo passado de uma vez
            )
